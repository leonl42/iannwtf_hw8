...
def test(model, test_data, loss_function, is_training,visual=True):
    ...
    accuracy_aggregator = []
    loss_aggregator = []
    for (input,orig_image, target) in test_data:
        # forward step
        prediction = model(input)
        # calculating loss
        loss = loss_function(orig_image, prediction)

        # add loss and accuracy to the lists
        loss_aggregator.append(loss.numpy())

        for t, p in zip(orig_image, prediction):
            accuracy_aggregator.append(
                tf.cast(my_tf_round(t, 1) == my_tf_round(p, 1), tf.float32))

        if visual:
            visualize_sample_img(orig_image,prediction)
            visual = False
...
def visualize_stat(train_losses,valid_losses,valid_accuracies):
    ...
    for j in range(1):
        axs[0,j].plot(train_losses)
        axs[0,j].plot(valid_losses)
        axs[1,j].plot(valid_accuracies)
        last_accuracy = valid_accuracies[-1].numpy()
